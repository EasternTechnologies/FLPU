...
//        dd($session->getValue('page'));
    ...
    private function hasAdminProperty () {
        ...
        foreach ( $this->adminProperties as $property ) {
            ...
            if ( isset($user->$property) || isset($user->$propertyCamel) || method_exists($user, $property) || method_exists($user, $propertyCamel) ) {
                return TRUE;
            }
        }
    ...
    public function visits ( Session $session ) {
        ...
        switch ( $sort ) {
            ...
            case 'admin':
                {
                    $users = Role::where('id', 1)->first()->users->pluck('id')->toArray();
                    $query->whereIn('user_id', $users);
                    break;
                }
            case 'analitic':
                {
                    $users = Role::where('id', 5)->first()->users->pluck('id')->toArray();
                    $query->whereIn('user_id', $users);
                    break;
                }
            case 'employee':
                {
                    $users = Role::where('id', 3)->first()->users->pluck('id')->toArray();
                    $query->whereIn('user_id', $users);
                    break;
                }
            case 'user':
                {
                    $users = Role::where('id', 4)->first()->users->pluck('id')->toArray();
                    $query->whereIn('user_id', $users);
                    break;
                }
            case 'manager':
                {
                    $users = Role::where('id', 2)->first()->users->pluck('id')->toArray();
                    $query->whereIn('user_id', $users);
                    break;
                }
        }
        ...
        if ( $name && $name != 'Гость' ) {
            $users_name = User::where('surname', 'like', '%' . $name . '%')
 ...
 */
        ...
        /*foreach ( $results as $date => $sessions_array ) {
            foreach ( $sessions_array as $user_id => $sessions) {
                if($user_id) $name = $users_all->where('id',$user_id)->first()->name.' '.$users_all->where('id',$user_id)->first()->surname.
                                     " \n".$users_all->where('id',$user_id)->first()->email; else $name = '';

                $data = Helper::logsInfo($sessions);

                $browser = '';
                if($sessions[0]->agent) {
                    $browser = $sessions[0]->agent->browser.' ('.$sessions[0]->agent->browser_version.')';
                }

                foreach ( $sessions as $singlsession ) {
                    //dump($user_id,$singlsession);
                    ...
                    $new_result[] = [
                      ...
                      'country'=>Helper::country($sessions[0]),
                      'device'=>Helper::device($sessions[0]),
                      'browser'=>$browser,
                      'categories'=>$data[0],
                      'count'=>Helper::logsCount($sessions),
                      'paths'=>$data[3],
        ...
        foreach ( $results as $key => $users_collection ) {
            foreach ( $users_collection as $id => $sessions ) {
                foreach ( $sessions as $item ) {
                    ...
                    $data = Helper::logsInfo($item);

                    $browser = '';
                    if ( $item->agent ) {
                        $browser = $item->agent->browser . ' (' . $item->agent->browser_version . ')';
                    }
                    ...
                    $new_result[] = [
                      ...
                      'id'         => $item->user_id,
                      'ip'         => $item->client_ip,
                      'country'    => Helper::country($item),
                      'device'     => Helper::device($item),
                      'browser'    => $browser,
                      'categories' => $data[ 0 ],
                      'count'      => Helper::logsCount($item),
                      'paths'      => $data[ 3 ],
                      'sum'        => gmdate('z:H:i:s', $data[ 1 ]),
                      'average'    => gmdate('z:H:i:s', $data[ 2 ]),
                      'session_id' => $item->id,
        ...
        return View::make('pragmarx/tracker::index')
                   ...
                   ->with('title', $title)
                   ->with('username_column', Tracker::getConfig('authenticated_user_username_column'))//->with('datatables_data', $datatables_data)
                   ->with('users_array', $users_all)
                   ->with('sort_array', $this->sort_array)
                   ->with('results', $paginate)
                   ->with('show_array', $this->show_array);
    }
...
//        $sheet->getStyle('A1')->getFill()->getStartColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_RED);
    ...
    public function summary_stats () {
        ...
        foreach ( $results as $id => $user ) {
            ...
            if ( $id ) {
                $name = $users_all->where('id', $id)->first()->name . ' ' . $users_all->where('id', $id)
                                                                                      ->first()->surname . " \n" . $users_all->where('id', $id)
                                                                                                                             ->first()->email;
            }
            else $name = '';
            $data         = Helper::logsInfosumm($user);
            $new_result[] = [
              'name'       => $name,
              'id'         => $id,
              'categories' => $data[ 0 ],
              'count'      => Helper::logsCountsumm($user),
              'paths'      => $data[ 3 ],
              'sum'        => gmdate('z:H:i:s', $data[ 1 ]),
              'average'    => gmdate('z:H:i:s', $data[ 2 ]),
            ];
        ...
        if ( !Input::get('ajax') )
            session(['start_period' => $start]);
        if ( !Input::get('ajax') )
            session(['end_period' => $end]);

        $title = 'Сводка статистики за период c ' . date('d.m.Y', strtotime($start)) . ' по ' . date('d.m.Y', strtotime($end));

        return View::make('pragmarx/tracker::index')
                   ->with('title', $title)
                   ->with('results', $new_result)
...
//                ->make(true);
    ...
    private function isAdmin () {
        ...
        foreach ( $this->adminProperties as $property ) {
            ...
            if ( ( isset($user->$property) && $user->$property ) || ( isset($user->$propertyCamel) && $user->$propertyCamel ) || ( method_exists($user, $property) && $user->$property() ) || ( method_exists($user, $propertyCamel) && $user->$propertyCamel() ) ) {
                return TRUE;
            }
        }
